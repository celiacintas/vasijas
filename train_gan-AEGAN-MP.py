...
checkpoint = torch.load("models/discriminador_v6_5000.pkl")
...
for epoch in tqdm.tqdm(range(epochs)):
    ...
    for batch, (image, labels, ids_name, tope, base) in enumerate(train_loader, 0):
        ...
        for i,k in enumerate(base):
            if k.min().item()!=0.0 or k.max().item()!=0.0:
                print('entro')
                crit_G_image += crit_G_image_func(predicted_r[i], image[i])
                
        
        loss_G =  crit_G(model_D(predicted).reshape(-1, 1), valid) + crit_G_image
    ...
    torch.save(model_G.state_dict(), 'models/generador_v6_current_{}.pkl'.format(epochs))
    torch.save(model_D.state_dict(), 'models/discriminador_v6_current_{}.pkl'.format(epochs))
...
torch.save(model_G.state_dict(), 'models/generador_v6_{}.pkl'.format(epochs))
torch.save(model_D.state_dict(), 'models/discriminador_v6_{}.pkl'.format(epochs))
