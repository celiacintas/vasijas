...
def finetune_vanilla_diffuser(image_dir, descriptions_file, config=CONFIG):
    ...
    tokenizer = CLIPTokenizer.from_pretrained(
        config["model_name"], subfolder="tokenizer"
    )
    ...
    optimizer = torch.optim.AdamW(
        unet.parameters(), lr=config["learning_rate"], weight_decay=0.01
    )
    ...
    for epoch in range(config["num_epochs"]):
        ...
        for batch_idx, batch in enumerate(progress_bar):
            ...
            timesteps = torch.randint(
                0, len(noise_scheduler), (latents.shape[0],), device=device
            )
