...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Generate denoising sequence from a finetuned checkpoint"
    )
    parser.add_argument(
        "--checkpoint",
        type=str,
    ...
    parser.add_argument(
        "--use-test-prompts",
        action="store_true",
        help="Use 10 random prompts from the test split instead of --prompt",
    )
    ...
    if args.use_test_prompts:
        print("Loading 10 random prompts from test split...")
        args.prompt = get_test_descriptions(
            image_dir="data/artifacts_with_descriptions/",
            descriptions_file="data/all_artifacts.json",
            train_ratio=0.7,
            n=10,
        )
        print(f"  Test prompts: {args.prompt}")
