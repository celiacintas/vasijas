...
def generate_images(
    prompts, models, num_inference_steps=50, guidance_scale=7.5, seed=None
):
    ...
    with torch.no_grad():
        for prompt_idx, prompt in enumerate(prompts, 1):
            ...
            # Initialize latents
            latents = torch.randn(
                (1, 4, 64, 64), device=device, dtype=text_embeddings.dtype
            )
...
def display_image_grid(images, cols=2, output_dir="generated_images", prefix=""):
    ...
    plt.tight_layout()
    plt.savefig(
        output_path / f"{prefix}generated_images_grid.png", dpi=100, bbox_inches="tight"
    )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Generate images from finetuned or base diffuser model"
    )
    parser.add_argument(
        "--checkpoint-dir",
        type=str,
    ...
    parser.add_argument(
        "--image-dir",
        type=str,
        default="data/cropped_artifacts",
        help="Path to image directory for test prompts",
    )
    parser.add_argument(
        "--descriptions-file",
        type=str,
        default="data/all_artifacts.json",
