...
checkpoint = torch.load("models/discriminador_v6c_5000.pkl")
...
crit_D = nn.BCELoss()
crit_G = nn.BCELoss()
crit_G_image = nn.MSELoss(reduction='sum')
...
for epoch in tqdm.tqdm(range(epochs)):
    ...
    for batch, (image, labels, ids_name, tope, base) in enumerate(train_loader, 0):
        ...
        for i,k in enumerate(base):
            if k.min()!=0 and k.max()!=0:
                crit_G_image += crit_G_image(predicted_r[i], image[i])
                
        
        loss_G =  crit_G(model_D(predicted).reshape(-1, 1), valid) + (crit_G_image * 2)
    ...
    torch.save(model_G.state_dict(), 'models/generador_v6c_current_{}.pkl'.format(epochs))
    torch.save(model_D.state_dict(), 'models/discriminador_v6c_current_{}.pkl'.format(epochs))
...
torch.save(model_G.state_dict(), 'models/generador_v6c_{}.pkl'.format(epochs))
torch.save(model_D.state_dict(), 'models/discriminador_v6c_{}.pkl'.format(epochs))
