...
imagenet_data.black_image = stats.rv_discrete(name='custm', values=([True, True], [0.5, 0.5]))
...
for epoch in tqdm.tqdm(range(epochs)):
    ...
    for batch, (image, labels, ids_name, tope, base, o_tope, o_base) in enumerate(train_loader, 0):
        ...
        predicted_r = model_G(origin)+o_tope
    ...
    torch.save(model_G.state_dict(), 'models/generador_v9_current_{}.pkl'.format(epochs))
    torch.save(model_D.state_dict(), 'models/discriminador_v9_current_{}.pkl'.format(epochs))
    
    for batch, (image, labels, ids_name, tope, base, o_tope, o_base) in enumerate(val_loader, 0):
...
torch.save(model_G.state_dict(), 'models/generador_v9_{}.pkl'.format(epochs))
torch.save(model_D.state_dict(), 'models/discriminador_v9_{}.pkl'.format(epochs))
